...
async def search(
    ...
    if mode == "semantic":
        passages = search_notes(str(current_user.id), clean_q, n_results=limit)
    ...
    query = query.where(
        or_(
            Note.title.ilike(wildcard_q),
            Note.refined_text.ilike(wildcard_q),
            Note.subject.ilike(wildcard_q),
            Note.unit.ilike(wildcard_q),
            Note.chapter.ilike(wildcard_q),
            Note.raw_ocr_text.ilike(wildcard_q)
        )
    )
