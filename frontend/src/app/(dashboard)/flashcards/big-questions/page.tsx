  ...
  useEffect(() => {
    const fetchNotes = async () => {
      try {
        ...
        setNotes(res.data);
        if (!selectedNoteId && res.data.length > 0) {
          ...
          <div key={i} className="card overflow-hidden transition-all duration-300">
            ...
            >
              <div className="space-y-1">
                <span className="text-xs font-bold uppercase tracking-wider text-brand-600 bg-brand-50 dark:bg-brand-900/30 px-2 py-0.5 rounded">
                  {q.marks} Marks
                </span>
              ...
              {expandedIndex === i ? <ChevronUp className="text-gray-400 shrink-0" /> : <ChevronDown className="text-gray-400 shrink-0" />}
            </button>
                ...
                <ul className="space-y-3">
                  {q.outline.map((step, si) => (
                    <li key={si} className="flex gap-3 text-sm text-gray-600 dark:text-gray-400">
                      <span className="flex-shrink-0 w-5 h-5 rounded-full bg-brand-100 dark:bg-brand-900/50 text-brand-700 dark:text-brand-300 flex items-center justify-center text-[10px] font-bold">
                        {si + 1}
                      </span>
                      {step}
                    </li>
